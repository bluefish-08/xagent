...
# Sentinel returned in place of any secret value in API responses. Sending it
# back on write means "keep the stored secret" (see _merge_masked_env).
_SECRET_MASK = "********"
            ...
            if config_field == "env" and value and isinstance(value, dict):
                ...
                for key in SENSITIVE_AUTH_FIELDS:
                    if key in encrypted_auth and encrypted_auth[key]:
                        ...
                        if encrypted_auth[key] == _SECRET_MASK:
...
def _mask_env(env: Any) -> dict:
    ...
    return {k: (_SECRET_MASK if v else v) for k, v in env.items()}
