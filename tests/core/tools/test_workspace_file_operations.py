...
class TestWorkspaceFileOperations:
    ...
    def test_list_all_user_files_description_forbids_discovery(
        ...
        # A cross-task file_id does not resolve under a scoped deployment, so the
        # description must not offer it as a read path — the model burns turns
        # retrying by name, path, and shell when it does.
